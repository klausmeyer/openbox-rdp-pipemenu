...
if($#ARGV + 1 == 1) {
  my $plain = $ARGV[0];
  my $crypted = "";
  # encrypt text using blowfish in 8 byte parts
  my $offset = 0;
  my $junk_size = 8;
  my $junk = "";
  do {
    $junk = substr($plain, $offset, $junk_size);
    if(length($junk) > 0) {
      $crypted .= $cipher->encrypt(blowfish_padding($junk));
      $offset += $junk_size;
    }
  }
  until(length($junk) < $junk_size);
  # encode crypted text in base64
  $crypted = encode_base64($crypted);
  say $crypted;
}
